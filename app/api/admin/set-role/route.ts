...
// Change a user's role. Requires team.manage.
export async function POST(req: NextRequest) {
  const orgId = await orgIdWithPermission("team.manage");
  if (!orgId) return Response.json({ error: "You don't have permission to manage the team." }, { status: 403 });
  let userId: string;
  let role: string;
  try {
  ...
  try {
    await setUserRole(orgId, userId, role as "customer" | "agent" | "admin");
